...
function generateSudoku() {
  ...
  const numGenerated = generateInput.value;
  ...
  const playableSudoku = removeNumbers(solvedSudoku, numGenerated);

  // Fill the grid with the playable puzzle
  fillSudokuGrid(playableSudoku);
}
...
function generateSolvedSudoku() {
  ...
  solveSudokuHelper(emptyBoard); // Solve an empty sudoku board
  return emptyBoard;
}

function removeNumbers(sudoku, numGenerated) {
  const gridSize = 9;
  let numRemoved = 0;

  while (numRemoved < numGenerated) {
    const row = Math.floor(Math.random() * gridSize);
    const col = Math.floor(Math.random() * gridSize);

    if (sudoku[row][col] !== 0) {
      const removedValue = sudoku[row][col];
      sudoku[row][col] = 0;

      // Check if the puzzle is still solvable with the number removed
      const tempBoard = JSON.parse(JSON.stringify(sudoku)); // Create a deep copy of the sudoku
      if (!solveSudokuHelper(tempBoard)) {
        // If the puzzle becomes unsolvable, revert the removal
        sudoku[row][col] = removedValue;
      } else {
        numRemoved++;
      }
    }
...
function fillSudokuGrid(sudoku) {
  const gridSize = 9;
